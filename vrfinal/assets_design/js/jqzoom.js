;(function($){
	$.fn.zoom = function(options){
		...
		var _option = {
			...
			thumb_image_width: 515,		// 当前展示图片的宽
			thumb_image_height: 350,	// 当前展示图片的高
			source_image_width: 900,  	// 放大图片的宽
			source_image_height: 612,	// 放大图片的高
		...
		if($ul.is("ul") && $ul.children("li").length && $ul.find(".bzoom_big_image").length){
			...
			if(!$(".bzoom_small_thumbs").length){
				var top = _option.thumb_image_height+5,
					width = _option.thumb_image_width,
					smwidth = (_option.thumb_image_width / _option.small_thumbs) - 5,
